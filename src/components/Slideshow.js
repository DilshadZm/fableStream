...
function Slideshow() {
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="slideshow-container mt-10">
            {Tales.map((items, i) => (
              ...
              >
                <div className="absolute inset-0 bg-cover bg-no-repeat bg-center" style={{ backgroundImage: `url(${Tales[index].image})` }}>
  <div className="p-4 bg-black bg-opacity-50 absolute top-0 left-0 h-full px-8 py-10 md:py-20 flex-col items-center justify-center">
    <h3 className="text-lg md:text-2xl  font-medium text-gray-50 mb-2 md:px-20 w-full">{Tales[index].title}</h3>
    <p className="text-sm md:text-base text-gray-50 mb-2 h-full py-10 md:px-28">{Tales[index].summery}</p>
  </div>
</div>
